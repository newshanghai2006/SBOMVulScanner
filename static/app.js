...
gitCommand.addEventListener('paste', event => {
  const plainText = event.clipboardData?.getData('text/plain');
  if (plainText == null) return;
  event.preventDefault();
  const normalized = plainText.replace(/[\r\n]+/g, ' ').trim();
  if (normalized.length > 4096) {
    generatorError.textContent = 'Git clone 命令不能超过 4096 个字符'; generatorError.hidden = false; return;
  }
...
async function generateSourceSbom() {
...
gitAuthForm.addEventListener('submit', event => {
  event.preventDefault();
  gitCredentials = {
    username: document.querySelector('#gitUsername').value.trim(),
    password: document.querySelector('#gitPassword').value,
  };
