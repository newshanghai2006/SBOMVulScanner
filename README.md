...
“导出报告”生成适合阅读、评审和归档的中文 Markdown 报告，结构包括：

1. 报告摘要和主要结论。
2. 按 `P0 - P3` 排列的风险概览与建议时限。
3. 每条漏洞的组件定位、CVSS、EPSS、KEV、修复版本、别名、公开摘要和参考来源。
4. 明确标记为未验证且不计入正式统计的 AI 新兴威胁情报。
5. SBOM 完整性、未命中边界、待确认和查询失败组件。
6. Excluded scope 结果。
7. 推荐实施顺序、扫描方法和判定边界。

报告只使用扫描结果中已有的结构化证据，不会自动编造依赖路径、利用条件、厂商结论或修复验证结果。需要这类项目上下文时，应由安全人员结合源码、构建依赖树和厂商公告补充复核。

## 多用户历史隔离

首次打开主页时，服务会为该浏览器配置文件签发随机会话 Cookie。Cookie 使用 `HttpOnly` 和 `SameSite=Strict`，SQLite 只保存令牌的 SHA-256 所有者标识，不保存原始令牌。以下数据均按浏览器会话隔离：
...
其他会话即使知道扫描 ID，也无法读取、下载或删除，接口返回 `404`。升级前无法归属到具体用户的共享历史和本地情报会迁移为 `legacy` 并对所有普通会话隐藏，不会错误分配给第一个访问者。

该方案用于无需账号系统的浏览器隔离。同一浏览器配置文件视为同一用户；不同浏览器、无痕窗口和不同操作系统账号会获得不同会话。清除站点 Cookie 后会得到新会话，无法再访问旧会话历史。若需要跨浏览器、跨设备恢复身份或精确到企业账号，应在 Nginx/反向代理接入统一认证，或继续增加正式用户账号系统。

会话 Cookie 在 HTTPS 下自动增加 `Secure`。多人使用或监听 `0.0.0.0` 时应通过 HTTPS 提供页面，避免 HTTP 网络中的 Cookie 被截获后冒用。
